...
- Source checkpoint: the Module 10 feature commit based on `bc4e782`; use Git history for its final content hash
- Modules 01-10: fully authored and Linux-validated, with thirty guided lessons and ten independent labs
- Modules 11-12: design scaffolds only; a scaffold is a reserved direction, not a playable module
...
- Field manual: Modules 01-10 have synchronized, self-contained chapters with complete guided source, local explanations, expected observations, troubleshooting, labs, and security models
- Current automated result: 35 tests passing and course attestation passing on Ubuntu 24.04.4 LTS arm64
...
The next task is Module 11 break/fix research. Module 10 composes cgroups, user/network namespaces, capability removal, Landlock, seccomp, Unix-socket capability mediation, clean credential inheritance, effective-state attestation, and synchronous exact-unit teardown. It introduced no object that can outlive its foreground owner, so the runtime registry did not need a new resource type.
